...
CFG <- list(
  version = "v1.3.1_upgrade_legacy_cause_master_covid_code_groups",
  ...
  covid_specific_icd10_regex = "^(U07[0-9]?|U09[0-9]?|U10[0-9]?)([A-Z0-9].*)?$",
...
tryCatch({
  ...
  if (file.exists(CFG$input_patch_csv)) {
    ...
    req_patch_levels <- c("cause_level", "cause_name", "level_1_name", "level_2_name", "level_3_name", "level_4_name")
    miss_patch_levels <- setdiff(req_patch_levels, names(patch))
    if (length(miss_patch_levels) > 0L) {
      stop("El patch maestro no trae columnas jerárquicas mínimas: ", paste(miss_patch_levels, collapse = ", "))
    }
    
    if (!"codigo_fuente" %in% names(patch)) patch[, codigo_fuente := NA_integer_]
    if (!"cie10" %in% names(patch)) patch[, cie10 := NA_character_]
    if (!"regex_r" %in% names(patch)) patch[, regex_r := NA_character_]
    
    patch[, cause_level := as.integer(cause_level)]
    patch[, codigo_fuente := suppressWarnings(as.integer(codigo_fuente))]
    patch[, path_key := make_path_key(level_1_name, level_2_name, level_3_name, level_4_name)]
    patch[, parent_path_key := fifelse(
      cause_level == 1L, "TOTAL",
      fifelse(cause_level == 2L,
              make_path_key(level_1_name, NA_character_, NA_character_, NA_character_),
              fifelse(cause_level == 3L,
                      make_path_key(level_1_name, level_2_name, NA_character_, NA_character_),
    ...
    patch_path_dups <- patch[!is.na(path_key), .N, by = path_key][N > 1L]
    if (nrow(patch_path_dups) > 0L) {
      fwrite(patch_path_dups, file.path(CFG$qc_dir, "qc_patch_duplicate_path_keys.csv"))
      stop("El patch maestro contiene path_key duplicados. Revisar qc_patch_duplicate_path_keys.csv")
    }
  }
  
  # ----------------------------------------------------------
  # IDs legacy + alineamiento con patch maestro
  # ----------------------------------------------------------
  ...
  if (!is.null(patch) && patch[!is.na(cause_concept_id), .N] > 0L) {
    next_id <- max(next_id, patch[!is.na(cause_concept_id), max(as.numeric(cause_concept_id))])
  }
  ...
  if (!is.null(patch)) {
    ...
    patch_id_by_path <- patch[!is.na(cause_concept_id) & !is.na(path_key), .(path_key, patch_cause_concept_id = cause_concept_id)]
    nodes <- merge(nodes, patch_id_by_path, by = "path_key", all.x = TRUE, sort = FALSE)
    nodes[!is.na(patch_cause_concept_id), cause_concept_id := patch_cause_concept_id]
    nodes[, patch_cause_concept_id := NULL]
    
    patch_nodes_missing <- patch[
      !is.na(cause_concept_id) &
      !is.na(path_key) &
      !path_key %in% nodes$path_key,
      .(
    ...
    if (nrow(patch_nodes_missing) > 0L) {
      nodes <- rbindlist(list(nodes, patch_nodes_missing), fill = TRUE, use.names = TRUE)
    }
  }
  
  if (nodes[, anyDuplicated(cause_concept_id)] > 0L) {
    dup_ids <- nodes[duplicated(cause_concept_id) | duplicated(cause_concept_id, fromLast = TRUE)][order(cause_concept_id)]
    fwrite(dup_ids, file.path(CFG$qc_dir, "qc_duplicate_cause_concept_ids_after_patch_alignment.csv"))
    stop("IDs de causa duplicados luego de alinear con patch maestro. Revisar qc_duplicate_cause_concept_ids_after_patch_alignment.csv")
  }
  ...
  if (!is.null(patch)) {
    ...
    if ("regex_r" %in% names(nodes)) {
      nodes[
        (is.na(icd10_regex) | icd10_regex == "") &
          !is.na(regex_r) & regex_r != "",
        icd10_regex := regex_r
      ]
    }
  }
  ...
  # ----------------------------------------------------------
  nodes[cause_concept_id == 9000600L & !is.na(icd10_regex),
        icd10_regex := sub("F(?:01|02|03", "F(?:00|01|02|03", icd10_regex, fixed = TRUE)]
  nodes[cause_concept_id == 9000940L & !is.na(icd10_regex),
        icd10_regex := sub("F(?:01|02|03", "F(?:00|01|02|03", icd10_regex, fixed = TRUE)]
  nodes[cause_concept_id == 9000380L & !is.na(icd10_regex),
        icd10_regex := sub("|P23[0-9]?|U04[0-9]?)",
                           "|P23[0-9]?|U04[0-9]?|U(?:07|09|10)[0-9]?)",
                           icd10_regex,
                           fixed = TRUE)]
  nodes[cause_concept_id == 9000190L & !is.na(cie10),
        cie10 := "B16-B19 (menos B17.1, B17.2, B17.8, B18.2, B18.8)"]
  nodes[cause_concept_id == 9000190L & !is.na(icd10_regex),
        icd10_regex := "^(?!B171$)(?!B172$)(?!B178$)(?!B182$)(?!B188$)(?:B(?:16|17|18|19)[0-9]?).*"]
  ...
  if (!"sex_restriction_target_default" %in% names(nodes)) {
    nodes[, sex_restriction_target_default := NA_character_]
  }
  if (!"target_age_start_default" %in% names(nodes)) {
    nodes[, target_age_start_default := NA_integer_]
  }
  if (!"target_age_end_default" %in% names(nodes)) {
    nodes[, target_age_end_default := NA_integer_]
  }
  ...
  # Restricciones duras por sexo para targets de redistribución claramente imposibles
  # epidemiológicamente. Esto no altera el mapeo directo observado; gobierna la
  # elegibilidad del target cuando un garbage code se redistribuye.
  female_target_ids <- c(
    9000420L, # Condiciones maternas
    9000710L, # Cáncer de cuello uterino
    9000720L, # Cáncer de cuerpo uterino
    9000730L  # Cáncer de ovario
  )
  male_target_ids <- c(
    9000740L, # Cáncer de próstata
    9000742L, # Cáncer de testículo
    9001280L  # Hiperplasia prostática benigna
  )
  nodes[cause_concept_id %in% female_target_ids, sex_restriction_target_default := "female"]
  nodes[cause_concept_id %in% male_target_ids, sex_restriction_target_default := "male"]
  
  # Restricciones duras por edad para targets con dominio etario estructural.
  # En este pipeline la edad está en años simples; por eso neonatal/lactante
  # se representa como age == 0.
  maternal_age_ids <- c(9000420L) # Condiciones maternas
  neonatal_age0_ids <- c(
    9000500L, # Complicaciones del parto prematuro
    9000510L, # Asfixia perinatal y trauma del parto
    9000520L, # Sepsis neonatal e infecciones
    9000530L, # Otras condiciones neonatales
    9001505L  # Síndrome de muerte súbita del lactante
  )
  nodes[cause_concept_id %in% maternal_age_ids, `:=`(
    target_age_start_default = 10L,
    target_age_end_default = 59L
  )]
  nodes[cause_concept_id %in% neonatal_age0_ids, `:=`(
    target_age_start_default = 0L,
    target_age_end_default = 0L
  )]
  ...
  preferred_order <- c(
    ...
    "redistribution_target_eligible_default", "sex_restriction_target_default",
    "target_age_start_default", "target_age_end_default", "redistribution_pool_default",
