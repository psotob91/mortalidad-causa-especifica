...
CFG <- list(
  ...
  qc_dir  = qc_dir_path("normalize_death_record"),
...
parse_date_multi <- function(x) {
  ...
  x_clean2 <- gsub("Set", "Sep", x_clean2, ignore.case = TRUE)
  
  # 4) parser explicito para dd-Mon-yy en espanol/mixto, sin depender del locale
  idx_mon_abbr <- is.na(out) & !is.na(x_chr) &
    grepl("^\\d{1,2}-[A-Za-z]{3}-\\d{2,4}$", x_chr)
  if (any(idx_mon_abbr)) {
    mon_map <- c(
    ...
    parsed_chr <- vapply(x_chr[idx_mon_abbr], function(val) {
      parts <- unlist(strsplit(val, "-", fixed = TRUE), use.names = FALSE)
      if (length(parts) != 3L) return(NA_character_)
      
      day <- suppressWarnings(as.integer(parts[1]))
      mon <- toupper(stringi::stri_trans_general(parts[2], "Latin-ASCII"))
      yr  <- suppressWarnings(as.integer(parts[3]))
      
      if (is.na(day) || is.na(yr) || !mon %in% names(mon_map)) return(NA_character_)
      if (yr < 100L) yr <- ifelse(yr <= 30L, 2000L + yr, 1900L + yr)
      
      sprintf("%04d-%s-%02d", yr, mon_map[[mon]], day)
    }, character(1))
    
    ok_chr <- !is.na(parsed_chr)
    if (any(ok_chr)) {
      out_idx <- which(idx_mon_abbr)[ok_chr]
      out[out_idx] <- as.IDate(parsed_chr[ok_chr], format = "%Y-%m-%d")
    }
  }
...
tryCatch({
  ...
  qc_summary <- data.table(
    metric = c(
      ...
      "n_rows_with_source_record_id_raw"
    ),
    value = c(
      nrow(final_dt),
  ...
  qc_source_record_id_profile <- final_dt[
    !is.na(source_record_id_raw) & trimws(source_record_id_raw) != "",
    .(
      n_rows = .N,
      n_unique_death_id = uniqueN(death_id),
      n_distinct_payload = uniqueN(
        paste(
          year_id, death_date, sex_id, age, location_id, icd10_ucod,
          sep = "||"
        )
      )
    ),
    by = .(source_file, source_record_id_raw)
  ][
    ,
    .(
      n_duplicate_groups = sum(n_rows > 1L),
      n_conflict_groups = sum(n_distinct_payload > 1L),
      n_rows_in_duplicate_groups = sum(n_rows[n_rows > 1L]),
      max_rows_per_source_record_id = max(n_rows)
    ),
    by = source_file
  ][order(source_file)]
  ...
  qc_source_record_id_path     <- file.path(CFG$qc_dir, "qc_source_record_id_profile.csv")
  ...
  fwrite(qc_source_record_id_profile, qc_source_record_id_path)
