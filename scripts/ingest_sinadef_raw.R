...
CFG <- list(
  ...
  input_dir = resolve_runtime_input_path(
    key = "sinadef_dir",
  ...
  qc_dir  = qc_dir_path("ingest_sinadef_raw"),
  ...
  min_sheet_match_count = 2L,
  min_detected_columns = 4L,
...
detect_best_sheet <- function(path, synonyms_flat) {
  ...
  if (length(sheets) == 1L) {
    return(list(
      ...
      score_table = data.table(sheet_name = sheets[1], match_count = NA_integer_)
    ))
  }
  ...
  score_dt <- data.table(
    sheet_name = sheets,
    match_count = as.integer(scores)
  )
  best_idx <- which.max(score_dt$match_count)
  
  if (length(best_idx) == 0L || is.na(score_dt$match_count[best_idx]) ||
      score_dt$match_count[best_idx] < CFG$min_sheet_match_count) {
    stop(
      "No se encontro una hoja con coincidencias minimas de encabezados en: ",
      basename(path),
  ...
  list(
    selected_sheet = score_dt$sheet_name[best_idx],
    score_table = score_dt
  )
}
...
file_inventory <- data.table(
  ...
  year_detected = extract_year_from_filename(files)
)
file_inventory[, accepted_flag := !is.na(year_detected) & year_detected %in% CFG$years_allowed]
file_inventory[, reject_reason := fifelse(
  is.na(year_detected),
  "year_not_detected_from_filename",
  fifelse(year_detected %in% CFG$years_allowed, NA_character_, "year_out_of_allowed_range")
)]

files <- file_inventory[accepted_flag == TRUE, file_path]
file_years <- file_inventory[accepted_flag == TRUE, year_detected]

if (length(files) == 0L) {
  stop("No se encontraron archivos con años válidos ", paste(CFG$years_allowed, collapse = ", "))
}

years_present <- sort(unique(file_years))
missing_years <- setdiff(CFG$years_allowed, years_present)
if (length(missing_years) > 0L) {
  stop(
    "Faltan archivos fuente para anios esperados en la ingesta: ",
...
qc_sheet_selection_list <- vector("list", length(files))
...
tryCatch({
  ...
  for (i in seq_along(files)) {
    ...
    if (ext %in% c("csv", "txt")) {
      ...
      qc_sheet_selection_list[[i]] <- data.table(
        file_name = f_name,
      ...
      sh_info <- detect_best_sheet(f, all_synonyms_flat)
      sh <- sh_info$selected_sheet
      ...
      qc_sheet_selection_list[[i]] <- copy(sh_info$score_table)[
        , `:=`(
          file_name = f_name,
          selected_flag = sheet_name == sh
        )
      ][, .(file_name, sheet_name, match_count, selected_flag)]
    ...
    n_detected_columns <- sum(!is.na(detected_clean))
    if (n_detected_columns < CFG$min_detected_columns) {
      stop(
        "Mapeo insuficiente de columnas en ", f_name,
        ": solo se detectaron ", n_detected_columns,
        " columnas canonicas."
      )
    }
    ...
    canonical_cols_for_blank <- c(
      "source_record_id_raw", "date_of_death", "sex_source_value",
  ...
  qc_sheet_selection_dt <- rbindlist(qc_sheet_selection_list, use.names = TRUE, fill = TRUE)
  ...
  qc_files_discovered_path       <- file.path(CFG$qc_dir, "qc_files_discovered_vs_accepted.csv")
  qc_sheet_selection_path        <- file.path(CFG$qc_dir, "qc_excel_sheet_selection.csv")
  ...
  fwrite(file_inventory, qc_files_discovered_path)
  fwrite(qc_sheet_selection_dt, qc_sheet_selection_path)
  ...
  for (p in c(
    ...
    qc_files_discovered_path,
    qc_sheet_selection_path
  )) {
    register_artifact(
      dataset_id = CFG$dataset_id,
      table_name = CFG$table_name,
