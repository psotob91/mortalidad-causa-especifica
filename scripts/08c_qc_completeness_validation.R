...
CFG <- list(
  ...
  input_candidates = c(
    here("data", "derived", "qc", "08_build_death_cause_final", "mortality_completeness_audit.csv"),
    here("data", "derived", "qc", "08_build_death_cause_final", "mortality_completeness_audit.parquet")
  ),
  
  qc_dir = here("data", "derived", "qc", "08c_qc_completeness_validation"),
...
tryCatch({
  ...
  fig2_dt[, age_f := factor(age_label, levels = make_age_label(sort(unique(age))))]
