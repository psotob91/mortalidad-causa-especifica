...
tryCatch({
  ...
  dt_raw <- as.data.table(read_auto(final_path))
  dt <- copy(dt_raw)
  ...
  if ("pandemic_reassigned_out_component" %in% names(dt_raw)) {
    ...
    dt[, pandemic_reassigned_out_component := 0]
  }
  
  if ("base_cause_deaths_corrected" %in% names(dt_raw)) {
    dt[, base_cause_deaths_corrected := as.numeric(dt_raw$base_cause_deaths_corrected)]
  } else {
    dt[, base_cause_deaths_corrected := deaths_post_redistribution * correction_factor_completeness]
  }
  ...
  qc_by_level <- out_final[, .(
    ...
    total_pandemic_reassigned_out = sum(pandemic_reassigned_out_component, na.rm = TRUE),
  ...
  leaf_before <- dt_leaf[, .(
    ...
    base_corrected_before = sum(base_cause_deaths_corrected, na.rm = TRUE),
    final_before = sum(deaths_final, na.rm = TRUE),
    pandemic_out_before = sum(pandemic_reassigned_out_component, na.rm = TRUE),
  ...
  leaf_after <- out_final[is_terminal == TRUE, .(
    ...
    base_corrected_after = sum(base_cause_deaths_corrected, na.rm = TRUE),
    final_after = sum(deaths_final, na.rm = TRUE),
    pandemic_out_after = sum(pandemic_reassigned_out_component, na.rm = TRUE),
  ...
  qc_total_consistency[, `:=`(
    ...
    diff_base_corrected = base_corrected_after - base_corrected_before,
    diff_final = final_after - final_before,
    diff_pandemic_out = pandemic_out_after - pandemic_out_before,
  ...
  qc_total_consistency[, `:=`(
    ...
    tol_base_corrected = pmax(CFG$qc_abs_tol, CFG$qc_rel_tol * pmax(abs(base_corrected_before), abs(base_corrected_after), 1)),
    tol_final = pmax(CFG$qc_abs_tol, CFG$qc_rel_tol * pmax(abs(final_before), abs(final_after), 1)),
    tol_pandemic_out = pmax(CFG$qc_abs_tol, CFG$qc_rel_tol * pmax(abs(pandemic_out_before), abs(pandemic_out_after), 1)),
  ...
  bad_mass <- qc_total_consistency[
    is.na(diff_obs) | is.na(diff_post) | is.na(diff_final) | is.na(diff_pandemic) |
      ...
      abs(diff_base_corrected) > tol_base_corrected |
      abs(diff_final) > tol_final |
      abs(diff_pandemic_out) > tol_pandemic_out |
  ...
  if (any(out_final$deaths_final < 0, na.rm = TRUE) ||
      ...
      any(out_final$base_cause_deaths_corrected < 0, na.rm = TRUE) ||
      any(out_final$pandemic_reassigned_out_component < 0, na.rm = TRUE) ||
