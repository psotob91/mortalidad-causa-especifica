...
# ------------------------------------------------------------
CFG <- list(
  version = "v1.0.2_exclude_covid_oprm_targets",
...
# Patch metodológico mínimo:
# cause_master excluye toda la familia J69* de respiratorias no-garbage.
# Si la regla de neumonitis cubre solo J69.0, J69.8/J69.9 quedan huérfanos
# como non-garbage unmapped downstream. Se expande explícitamente a J69*.
qc_rule_regex_patches <- ru0[
  source_group_name == "Pneumonitis" &
    regex_r == "^(J690)([A-Z0-9].*)?$",
  .(
    rule_id,
    source_group_code,
    source_group_name,
    regex_r_before = regex_r,
    regex_r_after = "^(J69[0-9]?)([A-Z0-9].*)?$",
    patch_reason = "Expandir cobertura de neumonitis a J69* para mantener consistencia con exclusiones J69 del cause_master"
  )
]
fwrite(qc_rule_regex_patches, file.path(CFG$qc_dir, "qc_rule_regex_patches.csv"))

ru0[
  source_group_name == "Pneumonitis" &
    regex_r == "^(J690)([A-Z0-9].*)?$",
  regex_r := "^(J69[0-9]?)([A-Z0-9].*)?$"
]
...
if (!"sex_restriction_target_default" %in% names(cm)) {
  cm[, sex_restriction_target_default := NA_character_]
}
cm[, sex_restriction_target_default := clean_chr(sex_restriction_target_default)]
if (!"target_age_start_default" %in% names(cm)) {
  cm[, target_age_start_default := NA_integer_]
}
if (!"target_age_end_default" %in% names(cm)) {
  cm[, target_age_end_default := NA_integer_]
}
cm[, target_age_start_default := safe_int(target_age_start_default)]
cm[, target_age_end_default := safe_int(target_age_end_default)]

forbidden_pandemic_target_ids <- cm[
  is_terminal == TRUE &
    (
      cause_name %in% c("COVID-19", "Other pandemic related mortality (OPRM)") |
        cause_code %in% c("COVID_19", "OPRM")
    ),
  unique(cause_concept_id)
]
...
ru_targets_expanded <- merge(
  ...
  cm[, .(
    ...
    target_sex_restriction_cm = sex_restriction_target_default,
    target_age_start_cm = target_age_start_default,
    target_age_end_cm = target_age_end_default,
    terminal_cause_name = cause_name,
    terminal_cause_code = cause_code
  )],
...
ru_targets_expanded[is.na(target_sex_restriction_cm), target_sex_restriction_cm := NA_character_]
ru_targets_expanded[, target_age_start_cm := safe_int(target_age_start_cm)]
ru_targets_expanded[, target_age_end_cm := safe_int(target_age_end_cm)]
ru_targets_expanded[, forbidden_pandemic_target := as.integer(
  terminal_cause_concept_id %in% forbidden_pandemic_target_ids
)]

ru_targets_expanded[, target_removed_by_patch := fifelse(
  target_eligible_cm == 0L | patch_exclude_target == 1L | forbidden_pandemic_target == 1L,
  1L, 0L
)]
...
qc_patch_targets_removed <- ru_targets_expanded[target_removed_by_patch == 1L, .(
  ...
  forbidden_pandemic_target
)]
fwrite(qc_patch_targets_removed, file.path(CFG$qc_dir, "qc_patch_targets_removed.csv"))

qc_target_rule_sex_conflict <- ru_targets_expanded[
  !is.na(sex_restriction) &
    !is.na(target_sex_restriction_cm) &
    sex_restriction != target_sex_restriction_cm,
  .(
    rule_id,
    source_group_code,
    source_group_name,
    ...
    terminal_cause_concept_id,
    terminal_cause_name,
    sex_restriction_rule = sex_restriction,
    sex_restriction_target = target_sex_restriction_cm
  )
]
fwrite(qc_target_rule_sex_conflict, file.path(CFG$qc_dir, "qc_target_rule_sex_conflict.csv"))

if (nrow(qc_target_rule_sex_conflict) > 0L) {
  stop("Hay conflictos entre sex_restriction de la regla y del target. Revisar qc_target_rule_sex_conflict.csv")
}

qc_target_rule_age_conflict <- ru_targets_expanded[
  (!is.na(age_start) | !is.na(age_end)) &
    (
      (!is.na(target_age_start_cm) & !is.na(age_end) & target_age_start_cm > age_end) |
        (!is.na(target_age_end_cm) & !is.na(age_start) & target_age_end_cm < age_start)
    ),
  .(
    rule_id,
    source_group_code,
    ...
    terminal_cause_concept_id,
    terminal_cause_name,
    age_start_rule = age_start,
    age_end_rule = age_end,
    age_start_target = target_age_start_cm,
    age_end_target = target_age_end_cm
  )
]
fwrite(qc_target_rule_age_conflict, file.path(CFG$qc_dir, "qc_target_rule_age_conflict.csv"))

if (nrow(qc_target_rule_age_conflict) > 0L) {
  stop("Hay conflictos entre age restriction de la regla y del target. Revisar qc_target_rule_age_conflict.csv")
}

qc_forbidden_pandemic_targets_removed <- qc_patch_targets_removed[
  forbidden_pandemic_target == 1L
]
fwrite(
  qc_forbidden_pandemic_targets_removed,
  file.path(CFG$qc_dir, "qc_forbidden_pandemic_targets_removed.csv")
)
...
# Reinyectar metadata canónica del target desde cause_master antes de exportar.
rules_final <- merge(
  rules_final[, !c("target_eligible_cm", "target_sex_restriction_cm", "target_age_start_cm", "target_age_end_cm")],
  cm[, .(
    target_cause_concept_id = cause_concept_id,
    target_eligible_cm = redistribution_target_eligible_default,
    target_sex_restriction_cm = sex_restriction_target_default,
    target_age_start_cm = target_age_start_default,
    target_age_end_cm = target_age_end_default
...
rules_final[is.na(target_eligible_cm), target_eligible_cm := 1L]
rules_final[, target_sex_restriction_cm := clean_chr(target_sex_restriction_cm)]
rules_final[, target_age_start_cm := safe_int(target_age_start_cm)]
rules_final[, target_age_end_cm := safe_int(target_age_end_cm)]
...
dict_dt <- data.table(
  ...
  description = c(
    ...
    "Elegibilidad por defecto del target según cause_master",
    "Restricción por sexo del target terminal según cause_master",
    "Bandera de exclusión aplicada por patch de targets",
    "Indicador de remoción efectiva del target tras aplicar elegibilidad y patch"
  ),
