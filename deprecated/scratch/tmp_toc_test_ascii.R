...
label_map<-c(anio="Anio", n_muertes_ambos="N ambos", n_muertes_hombre="N hombres", n_muertes_mujer="N mujeres", tasa_ambos_100000="Tasa ambos", tasa_hombre_100000="Tasa hombres", tasa_mujer_100000="Tasa mujeres")
ft<-flextable(out)
ft<-do.call(set_header_labels, c(list(x=ft), as.list(label_map)))
...
doc<-read_docx()
doc<-body_add_par(doc, "Informe descriptivo", style="heading 1")
doc<-body_add_par(doc, "Indice general", style="heading 1")
doc<-body_add_toc(doc, level=2)
doc<-body_add_break(doc)
doc<-body_add_par(doc, "Introduccion", style="heading 1")
doc<-body_add_par(doc, "Texto de prueba.", style="Normal")
doc<-body_add_par(doc, "Totales", style="heading 1")
doc<-body_add_par(doc, "Tabla T1. Mortalidad total nacional 2018-2024", style="heading 3")
doc<-body_add_flextable(doc, ft)
print(doc, target="reports/all_causes_word_report/test_t1_with_toc_ascii.docx")
