...
#      Un PDF único multipágina para mortalidad por edad.
#   2) mortality_time_all_levels.pdf
#      Un PDF único multipágina para mortalidad por año.
#
# En ambos PDFs:
...
# ------------------------------------------------------------
CFG <- list(
  version = "v0.3.0_mortality_only_final_test",
  ...
  input_roughness_candidates = qc_path_candidates("build_mortality_rates", "qc_temporal_roughness.csv"),
  
  out_dir = here("outputs", "diagnostic_pdfs"),
  qc_dir  = qc_dir_path("diagnostic_mortality_pdfs"),
...
prepare_dataset <- function(location_id_target = CFG$location_id_national,
  ...
  if ("cause_name" %in% names(mort)) mort[, cause_name := NULL]
  ...
  mort[is.na(cause_name) | cause_name == "", cause_name := paste0("cause_", cause_concept_id)]
  
  mort[, sex_label := sex_label_fun(sex_id)]
  mort <- pretty_cause_label(mort, width = CFG$wrap_width_cause)
...
get_level_cause_ids <- function(prep, cause_level_target, top_n = NULL, sort_mode = c("concept_id", "name", "deaths_desc")) {
  sort_mode <- match.arg(sort_mode)
  
  x <- filter_causes_auto(
    prep$cause_stats,
    cause_level_target = cause_level_target,
    include_total = (cause_level_target == 0L),
    top_n = NULL,
    cm = prep$cm
  )
  
  if (cause_level_target == 1L) {
...
plot_age_distribution <- function(dt,
  ...
  compose_page_with_caption(
    ...
    caption = paste(
      ...
      "Revise picos neonatales, mortalidad infantil, ascenso en edades mayores, diferencias hombre-mujer y quiebres inesperados por edad o año."
    ),
    ncol = 4L
  )
}
...
plot_time_trend <- function(dt,
  ...
  compose_page_with_caption(
    ...
    caption = paste(
      ...
      "Busque saltos abruptos, inversiones inesperadas por sexo, patrones pandémicos 2020-2021 o serruchos incompatibles con el suavizado esperado."
    ),
    ncol = 2L
  )
}
