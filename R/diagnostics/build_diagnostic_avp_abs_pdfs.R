...
# ============================================================
# diagnostico_avp_abs_pdfs.R
# ------------------------------------------------------------
# PDFs diagnósticos SOLO para AVP absoluto reconciliado
# en Perú 2018-2024.
# ============================================================
...
CFG <- list(
  version = "v0.1.1_avp_abs_only_final",
  dataset_id = "diagnostic_pdfs_avp_abs_only",
  ...
  input_qc_avp_candidates = qc_path_candidates("compute_avp_yll", "qc_rate_plausibility.csv"),
  
  out_dir = here("outputs", "diagnostic_pdfs"),
  qc_dir  = qc_dir_path("diagnostic_avp_abs_pdfs"),
...
# Mantiene eje lineal y evita notación científica.
# Usa cortes SI modernos compatibles con scales recientes.
avp_axis_labeller <- function() {
  scales::label_number(
    accuracy = 0.1,
...
prepare_dataset <- function(location_id_target = CFG$location_id_national,
  ...
  if (!is.na(col_avp_rate) && col_avp_rate != "avp_rate") {
    setnames(avp, col_avp_rate, "avp_rate")
  }
  
  req_avp <- c(
    "year_id", "location_id", "sex_id", "age", "cause_concept_id",
    "cause_level", "population", "avp_abs"
  )
  ...
  if ("avp_rate" %in% names(avp)) {
    avp[, avp_rate := as.numeric(avp_rate)]
  }
  ...
  cause_stats <- avp[, .(
    ...
    max_avp_abs = suppressWarnings(max(avp_abs, na.rm = TRUE)),
    n_cells = .N
  ), by = cause_concept_id]
...
filter_causes_auto <- function(cause_stats,
  ...
  x[is.na(max_avp_abs), max_avp_abs := 0]
...
compute_qc_flags <- function(prep) {
  ...
  qc_abs <- avp[, .(
    n_negative = sum(avp_abs < 0, na.rm = TRUE),
    max_avp_abs = suppressWarnings(max(avp_abs, na.rm = TRUE)),
    n_hard = sum(avp_abs > CFG$qc_abs_hard_max, na.rm = TRUE)
  ...
  avp_all_age[, lag_abs := shift(avp_abs), by = .(cause_concept_id, sex_id)]
  avp_all_age[, ratio_jump := fifelse(!is.na(lag_abs) & lag_abs > 0, avp_abs / lag_abs, NA_real_)]
  ...
  wide_sex <- dcast(
    avp[, .(avp_abs = sum(avp_abs, na.rm = TRUE)),
  ...
  impossible_age <- avp[
    age < CFG$age_min | age > CFG$age_max | avp_abs < 0,
    .N,
    by = .(cause_concept_id, cause_name, cause_level)
  ]
  
  fwrite(qc_abs, file.path(CFG$qc_dir, "qc_diag_avp_abs_flags.csv"))
...
plot_age_distribution <- function(dt,
  ...
  plots <- lapply(cause_ids, function(cid) {
    ...
    ggplot() +
      geom_line(
        ...
        aes(x = age, y = avp_abs, group = sex_label, color = sex_label, linetype = sex_label),
        linewidth = 0.35, alpha = 0.55
      ) +
      geom_line(
        data = xc[sex_label == "Hombre"],
        aes(x = age, y = avp_abs, group = sex_label, color = sex_label),
      ...
      scale_y_continuous(labels = avp_axis_labeller()) +
      labs(
        title = cause_lab,
        x = "Edad simple",
        y = "AVP absoluto",
  ...
  compose_page_with_caption(
    ...
    caption = paste(
      ...
      "El eje Y muestra AVP absoluto real en escala lineal; solo se abrevia el etiquetado para facilitar lectura.",
      "Revise concentración de AVP en edades tempranas versus tardías, diferencias hombre-mujer, picos por lesiones o causas perinatales, quiebres inesperados por edad o año, y causas con gran volumen absoluto de pérdida de años de vida."
    ),
    ncol = 4L
  )
}
...
plot_time_trend <- function(dt,
  ...
  plots <- lapply(cause_ids, function(cid) {
    ...
    ggplot() +
      geom_line(
        data = xc[sex_label == "Ambos"],
        aes(x = year_id, y = avp_abs, group = sex_label, color = sex_label),
        linewidth = 0.7, alpha = 0.7
      ) +
      geom_point(
        data = xc[sex_label == "Ambos"],
        aes(x = year_id, y = avp_abs, color = sex_label),
        size = 1.1, alpha = 0.7
      ) +
      geom_line(
        data = xc[sex_label == "Hombre"],
  ...
  compose_page_with_caption(
    ...
    caption = paste(
      ...
      "El eje Y muestra AVP absoluto real en escala lineal con etiquetas abreviadas.",
      "Busque saltos abruptos, inversiones inesperadas por sexo, patrones pandémicos 2020-2021, serruchos incompatibles con la trayectoria esperada del AVP, o causas con gran volumen absoluto persistente."
    ),
    ncol = 2L
  )
}
...
build_pdf_avp_abs_age_all_levels <- function(prep,
                                             out_file = file.path(CFG$out_dir, "avp_abs_age_all_levels.pdf"),
  ...
  for (lvl in c(4L, 3L, 2L, 1L, 0L)) {
    ...
    lev_plots <- lapply(seq_along(pages), function(i) {
      plot_age_distribution(
        dt = prep$avp,
        cause_ids = pages[[i]],
        title = paste0("AVP absoluto por edad - nivel ", lvl),
...
build_pdf_avp_abs_time_all_levels <- function(prep,
                                              out_file = file.path(CFG$out_dir, "avp_abs_time_all_levels.pdf"),
                                              causes_per_page = CFG$causes_per_page,
                                              top_n_per_level = NULL,
                                              sort_mode = "concept_id") {
  plot_list <- list()
  ...
  for (lvl in c(4L, 3L, 2L, 1L, 0L)) {
    ...
    lev_plots <- lapply(seq_along(pages), function(i) {
      plot_time_trend(
        dt = prep$avp,
        cause_ids = pages[[i]],
        title = paste0("AVP absoluto por año - nivel ", lvl),
        subtitle = paste0("Nivel ", lvl, " | página ", i, " de ", length(pages),
...
run_iterative_diagnostics <- function(test_mode = TRUE) {
  ...
  if (isTRUE(test_mode)) {
    msg("Modo iterativo TEST: generando PDFs únicos de AVP absoluto con subconjunto de causas.")
    ...
    build_pdf_avp_abs_age_all_levels(
      prep,
      out_file = file.path(CFG$out_dir, "TEST_avp_abs_age_all_levels.pdf"),
      top_n_per_level = 8L,
      sort_mode = "concept_id"
    )
    
    build_pdf_avp_abs_time_all_levels(
      prep,
      out_file = file.path(CFG$out_dir, "TEST_avp_abs_time_all_levels.pdf"),
      top_n_per_level = 8L,
      sort_mode = "concept_id"
    )
  ...
  msg("Modo FULL: generando PDFs únicos de AVP absoluto con todas las causas.")
